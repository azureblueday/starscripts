...
local ConfigFolder = "Nexus_Configs"
...
local ThemePresets = {
    ["Nexus (Default)"] = {
        Background = Color3.fromRGB(138, 0, 230), 
        Sidebar = Color3.fromRGB(153, 102, 255), 
        Element = Color3.fromRGB(32, 32, 42),
        Text = Color3.fromRGB(245, 245, 255), 
        TextDim = Color3.fromRGB(160, 160, 180),
        Accent1 = Color3.fromRGB(140, 20, 255), 
...
local function UpdateTheme(themeName)
    local newTheme = ThemePresets[themeName] or ThemePresets["Arctic Frost"]
...
function Library:Window(title, iconId, footerText, useTooltips)
    if CoreGui:FindFirstChild("NexusV36") then CoreGui.NexusV36:Destroy() end
    ScreenGui = Instance.new("ScreenGui") ScreenGui.Name="NexusV36"
    ...
    RunService.RenderStepped:Connect(function()
        ...
        -- Плавное покачивание угла градиента
        BgGradient.Rotation = 45 + math.sin(t * 0.5) * 15
        
        -- Обновляем цвета динамически из темы (чтобы при смене темы фон менялся)
        -- Перелив от цвета Фона к цвету Сайдбара (создает приятный эффект глубины)
    ...
    VersionLabel.Text = footerText or "Nexus V1" 
