...
local Luna = { 
	...
	ThemeGradient = ColorSequence.new{
		ColorSequenceKeypoint.new(0.00, Color3.fromRGB(199, 130, 242)), 
		ColorSequenceKeypoint.new(0.50, Color3.fromRGB(221, 130, 238)), 
		ColorSequenceKeypoint.new(1.00, Color3.fromRGB(243, 129, 233))
	}
}
local IconModule = {
	Lucide = nil,
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		Luna:Notification({Title = "Interface Hidden", Content = "Press "..tostring(bind).." to reopen.", Icon = "visibility_off"})
	end
...
function Luna:Notification(data)
	task.spawn(function()
		...
		newNotification.BackgroundColor3 = DarkTheme.Secondary
		...
		task.wait(0.15)
		TweenService:Create(newNotification, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.15}):Play()
		TweenService:Create(newNotification.Title, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {TextTransparency = 0}):Play()
		task.wait(0.05)
		TweenService:Create(newNotification.Icon, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {ImageTransparency = 0}):Play()
		task.wait(0.05)
		TweenService:Create(newNotification.Description, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {TextTransparency = 0.35}):Play()
		TweenService:Create(newNotification.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Transparency = 0.85}):Play()
...
local function Unhide(Window, currentTab)
	...
	Window.Visible = true
	task.wait()
	tween(Window, {BackgroundTransparency = 0.05})
	tween(Window.Elements, {BackgroundTransparency = 0.02})
...
function Luna:CreateWindow(WindowSettings)
	...
	Main.BackgroundColor3 = DarkTheme.Background
	Main.Size = MainSize
	...
	LoadingFrame.BackgroundColor3 = DarkTheme.Background

	tween(Elements.Parent, {BackgroundTransparency = 1})
	Elements.Parent.Visible = false
	Elements.Parent.BackgroundColor3 = DarkTheme.Secondary
	...
	Navigation.BackgroundColor3 = DarkTheme.Secondary
	...
	if WindowSettings.KeySystem then
		...
		if not Passthrough then
			...
			if typesys == "Discord" then Btn = KeySystem.Action.Discord end

			local AttemptsRemaining = math.random(2, 5)
			KeySystem.Visible = true
			KeySystem.BackgroundColor3 = DarkTheme.Background
			...
			if KeySettings.SecondAction.Enabled == true then Btn.Visible = true end
				...
				if typesys == "Discord" then
					...
					if request then
						request({
							...
							Body = HttpService:JSONEncode({cmd = 'INVITE_BROWSER', nonce = HttpService:GenerateGUID(false), args = {code = KeySettings.SecondAction.Parameter}})
						})
					end
				else
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then writefile(direc .. WindowSettings.KeySettings.FileName .. ".luna", FoundKey) end
						Luna:Notification({Title = "Key System", Content = "Key saved successfully.", Icon = "lock_open"})
					end
				else
